...
export class TransactionBuilder {
    ...
    setProcessor(processor = undefined) {
        if (processor == null)
            return;
        this.processor = processor;
    ...
    setContract(contract = undefined) {
        if (contract == null)
            return;
        this.contract = contract;
        this.payload.setContractName(this.contract);
    }
    setFunctionName(func = undefined) {
        if (func == null)
            return;
        this.func = func;
        this.payload.setFunctionName(this.func);
    }
    setStamps(stamps = undefined) {
        if (stamps == null)
            return;
        this.stamps = stamps;
    ...
    sign(sk) {
        ...
        if (wallet.verify(this.sender, this.payloadBytes, this.signature))
            console.log(true);
        else
            console.log(false);
    }
    setSignature() {
        ...
        if (!this.transactionSigned)
            throw new Error(`No signature present. Use the "sign" method then try again.`);
